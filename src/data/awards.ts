import { Award } from "@/types";

export const awards: Award[] = [
  {
    title: "Theia Scientific Award",
    organization: "2025 Microscopy Hackathon",
    date: "2025-11",
    description:
      "Awarded for NanoRange, an AI-powered nanoparticle analysis platform using multiple specialized agents to detect, measure, and visualize nanoparticles from microscopy images.",
    url: "https://kaliningroup.github.io/mic_hackathon_2/awards/",
    projectSlug: "nanorange-v1",
  },
  {
    title: "DENS Mystery Award",
    organization: "2025 Microscopy Hackathon",
    date: "2025-11",
    description:
      "Second award for NanoRange at the same hackathon, recognizing the platform's innovative approach to automated microscopy analysis.",
    url: "https://kaliningroup.github.io/mic_hackathon_2/awards/",
    projectSlug: "nanorange-v1",
  },
  {
    title: "4th Place: Prize by Lila Sciences",
    organization: "2025 Hackathon on LLMs for Materials Science & Chemistry",
    date: "2025-09",
    description:
      "CrystaLenz ranked 4th among top-scoring projects in a global competition with over 1,000 participants across 100+ teams globally.",
    url: "https://llmhackathon.github.io/awards/",
    projectSlug: "crystallenz",
  },
  {
    title: "1st Place: SPE Datathon 2024",
    organization: "Society of Petroleum Engineers (SPE)",
    date: "2024-10",
    description:
      "Designed and implemented ML models (Random Forest, SARIMA) to forecast carbon emissions and support CCUS decision-making from large-scale climate data.",
    url: "https://www.linkedin.com/feed/update/urn:li:activity:7254588094333493248/",
    sourceUrl: "https://github.com/SarupaDebnath/SPEDatathon2024",
  },
];
